...
static char DefaultTitle[]="WinAdapt V1.1";
static const char* WindowTitle=DefaultTitle;
static unsigned UpdateInterval=200;
...
void setUpdateInterval(const unsigned Interval)
{
...
{
    VtlInit();
    static const char* szAppName = WindowTitle ;
    HWND        hwnd ;
    MSG         msg ;
    WNDCLASS    wndclass ;
    if (!hPrevInstance)
                           ...
                           IN WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_CREATE :
        SetTimer(hwnd, SIMPROTIMER, UpdateInterval, NULL);
        return 0;
    case WM_TIMER :
...
{
    // Code from msdn; error handling removed
    HANDLE hTimer = NULL;
    LARGE_INTEGER liDueTime;
    liDueTime.QuadPart=-1e7*amount; // negativer Wert= relative Zeit
    // Create a waitable timer.
    hTimer = CreateWaitableTimer(NULL, TRUE, "WaitableTimer");
    // Set timer to wait for amount time.
...
int main( int   argc, char *argv[] )
{
    ...
    g_timeout_add(UpdateInterval, (GSourceFunc) timeout_callback, NULL);
...
void delay(double amount)
{
    ...
    t.tv_nsec=long((amount-(double)t.tv_sec)*1e9);
    nanosleep(&t, nullptr);
}
...
#include <thread>
extern void runAsThread(void (*TheFunction)())
{
    thread t(TheFunction);
    t.detach();
}
